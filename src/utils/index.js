import { forEach, get, has, head, includes, isNumber, toLower } from "lodash";
...
const showError = ({ e, setError = (e) => console.log(e), isField = true, errorsKey = "response.data.errors" }) => {
  forEach(get(e, errorsKey, []), (error) => {
...
const getIconName = (module = "") => {
  switch (module) {
    ...
    case "ADMIN_GROUP":
      return "icon-single-user";
    case "ADMIN_MENTOR":
      return "icon-single-user";
    case "ADMIN_STUDENT":
      return "icon-single-user";
    case "ADMIN_SEMESTERS":
      return "icon-semesters";
    case "ADMIN_MC":
    case "STUDENT_MC":
      return "icon-petition";
    case "ADMIN_SEND_MESSAGE":
      return "icon-send-message";
    case "STUDENT_ATTENDANCE":
    case "ADMIN_ATTENDANCE":
    case "MENTOR_ATTENDANCE":
      return "icon-check-out";
    case "MODULES":
      return "icon-courses";
    default:
      return "icon-home";
...
const numberPrettier = (number, type = "comma") => {
  if (type === "comma") return isNumber(number) ? Number(number).toLocaleString() : "";
  return new Intl.NumberFormat("fr-FR", { currency: "UZS" }).format(number);
};
