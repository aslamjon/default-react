...
const Select = ({
    ...
    (index) => {
      if (editable) {
        ...
        if (isMulti) {
          ...
          setOptionsState((s) =>
            s.map((item, i) => (item[valueKey] === unSelected[valueKey] ? { ...item, isSelected: false } : item))
          );
  ...
  const handlingKye = useCallback(
    (e) => {
      switch (e.keyCode) {
        case 13:
          e.preventDefault();
          if (e.target.value !== "" && (get(action, "create", false) || get(action, "edit", false)))
            createButtonHandling(e.target.value);
      }
  ...
  useEffect(() => {
    ...
    // if (isMulti && isArray(value)) values = value?.map((id) => options?.find((item) => item[valueKey] === id));
    // else
    if (!isMulti) {
      values = options?.find((i) => i[valueKey] == (isArray(value) ? head(value) : value));
      setSelected(values ?? "");
    }
  }, [value]);
  ...
  return (
    ...
    >
      <div
        ref={headerRef}
        className={`select__header ${!isMulti && !isEmpty(selected) && "optionSelected"} ${disabled && "disabled"}`}
      >
        <CustomHeader
          {...{
            isMulti,
          ...
          <OutsideClickHandler onOutsideClick={clickOutside}>
            <div>
              ...
              {!isSearchable && viewSelectOption && (!get(action, "create") || options.length <= 19) && (
                <input
                  type="text"
                  onKeyDown={bodyOnKeyDownHandling}
                  autoFocus={!isSearchable && viewSelectOption}
